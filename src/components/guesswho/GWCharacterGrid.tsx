        ...
        {characters.map((player, i) => {
          ...
          return (
            <motion.button
              ...
              disabled={disabled}
              className={`relative overflow-hidden rounded-lg border-2 transition-all duration-200 group text-left ${
                isEliminated
                  ? "border-cricket-red/50"
                  : `hover:scale-[1.03] hover:shadow-lg shadow-md ${config.border} ${config.glow}`
              } ${
                disabled
                  ? "cursor-default"
                  : "cursor-pointer"
            ...
            >
              ...
              {isEliminated && (
                <motion.div
                  initial={{ opacity: 0 }}
                  animate={{ opacity: 1 }}
                  className="absolute inset-0 z-10 flex items-center justify-center bg-cricket-dark/85 backdrop-blur-[3px]"
                >
                  <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-full bg-cricket-red/30 flex items-center justify-center">
